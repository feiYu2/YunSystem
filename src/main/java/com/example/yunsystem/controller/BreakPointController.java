...
public class BreakPointController {
    ...
    public Object checkFileMd5(Md5 myMd5,String username,String fileSize) throws IOException, URISyntaxException, InterruptedException {
        ...
        List<UserStore>  Username = userStoreDao.findByName(username+"@qq.com");

        UserStore byUsername =Username.get(0);
        ...
        List<Md5> byMd5 =  md5Dao.findByMd5AndPathAndFilename(myMd5.getFileMd5(), myMd5.getPath(), myMd5.getFileName());
        Md5 byMd5AndPathAndFilename =null;
        if(byMd5.size()!=0){
            byMd5AndPathAndFilename = byMd5.get(0);
        }
        ...
        if (processing) {
            //该文件已经上传过了 —— 下面所有的功能都是秒传
            //先判断这次上传的文件在当前路径下是否存在
            List<Md5> byMd5AndPath = md5Dao.findByMd5AndPath(myMd5.getFileMd5(), myMd5.getPath());
           ...
           // List<Md5> byMd5AndFilename = md5Dao.findByMd5AndFilename(myMd5.getFileMd5(), myMd5.getFileName());
            //从文件系统中找到一个Md5值相同的文件
            List<Md5> byFileMd5 = md5Dao.findByFileMd5(myMd5.getFileMd5());
            Md5 tmpMd5 = new Md5();
            ...
            if (byMd5AndPath.size() == 0){ //这次想要上传的文件在当前路径下没有 —— 不同的路径上传
                ...
                if (!myMd5.getFileName().equals(tmpMd5.getFileName())){ // 同一个文件，文件名修改了，内容没变
                    ...
                    if (b){
                        ...
                        // 先重命名
                        JsonResult rename = fileService.rename(myMd5.getPath() + "/" + tmpMd5.getFileName(), myMd5.getFileName());
                    }
                    else
                ...
                if (byMd5AndPathAndFilename== null){ // 同一个文件，文件名修改了，内容没变
                    ...
                    //下面重命名文件
                    fileService.rename(dirPath+"/"+tmpMd5.getFileName(),myMd5.getFileName());

                    //再移动到当前上传的路径下
                    String[] oldDirpaths = {dirPath+"/"+myMd5.getFileName()};
                    fileService.move(oldDirpaths,myMd5.getPath());
