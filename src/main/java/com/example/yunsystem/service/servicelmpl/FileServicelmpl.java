...
import com.example.yunsystem.entry.Dir;
...
public class FileServicelmpl implements FileService {
    ...
    @Override
    public JsonResult mkdirMulu(String pPath, String fileName,String email) throws URISyntaxException {
        //拼接成创建文件夹的hdfs全路径
        ...
        try {
            ...
            if (fileSystem.exists(newHdfsPath)) {
                ...
                if (IsDir != null){
                    ...
                    //修改数据库中对于数据的filename
                    List<Md5> member = new ArrayList<>();
                    String oldpath = oldPath.substring(0,oldPath.lastIndexOf("/"));
                    member = md5Dao.query(oldpath,oldFileName);
                    if(member.size()==0){
                        result.setStatus("文件不存在！");
                    }
                    Md5 mem = new Md5();
                    mem.setId(member.get(0).getId());
                    mem.setPath(oldFatherPath);
                    mem.setFileName(newName);
                    mem.setUpdateTime(new Date());
                    int i = md5Dao.updateFileName(mem);
                }
            }
        ...
        for (String fileDelPath : fileDelPaths) {
            ...
            try {
                ...
                if (!fileSystem.exists(deletePath)) {
                    ...
                    String path = fileDelPath.substring(0,fileDelPath.lastIndexOf("/"));
                    String filename = fileDelPath.substring(fileDelPath.lastIndexOf("/")+1);
    ...
    @Override
    public ResponseEntity<JsonResult> upload(MultipartFile file, String mulupath,String email) throws URISyntaxException {
        JsonResult result = new JsonResult();
        if (!file.isEmpty()) {
            try {
                ...
                String dataLocalName = email.substring(0,email.indexOf("@"));

                File userFolder = new File(path+"\\"+dataLocalName);
                ...
                if(result.getStatus().equals("上传成功！")){
                    Md5 member = new Md5();
                    member.setUid(email);
                    member.setUsername(email);
                    member.setPath(mulupath);
                    member.setFileName(fileName);
                    member.setCreateTime(new Date());
                    md5Dao.insert(member);
                }
    ...
    public void decompress(String path) throws URISyntaxException {
        ...
        try {
            ...
            String outputuri = path.substring(0,path.indexOf("."));
    ...
    public String Type(String type) {
        ...
        if (type.equals("wmv")||type.equals("avi")|| type.equals("mov") || type.equals("swf")|| type.equals("mpg")|| type.equals("mp4")){
            return "video";
        }
        ...
        if (type.equals("flac")||type.equals("aac")||type.equals("wam")||type.equals("wav")|| type.equals("aif")||type.equals("mp3")|| type.equals("m4a")||type.equals("ram")|| type.equals("wma")){
            return "music";
        }
        if (type.equals("txt")){
            return "txt";
