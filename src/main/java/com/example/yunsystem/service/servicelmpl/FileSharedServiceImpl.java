...
import com.example.yunsystem.dao.repository.FileSharedRepository;
...
import com.example.yunsystem.service.FileSharedService;
...
public class FileSharedServiceImpl implements FileSharedService{
    ...
    @Override
    public Map<String, Object> CreateSharedLink(String[] paths, String ifPasswd,String email) throws URISyntaxException, IOException {
         ...
         * 测试的时候要更换name
         * **/
        shareDetails.setUsername(email);
//        shareDetails.setUsername("lww");
    ...
    @Override
    public List<ShareDetails> AllShare(String email) {

        return shareDetailsRepository.findAllByUsername(email);
    }
    ...
    public JsonShare ToShare(String id, String passwd) throws IOException {
        ...
        user = userInforDao.query(name);


        jsonShare.setUsername(user.get(0).getName());//分享者的名字
        jsonShare.setVip(user.get(0).getRole().toString());//分享者的用户等级
        jsonShare.setUserIntro("");//分享者的个性签名
